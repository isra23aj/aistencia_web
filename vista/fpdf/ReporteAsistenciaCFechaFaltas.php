...
if($empleado =="todos"){
   ...
   DATE_FORMAT(asistencia_continuo.entrada, '%m-%d-%Y %H:%I:%s') AS 'entrada',
   DATE_FORMAT(asistencia_continuo.salida, '%m-%d-%Y %H:%I:%s') AS 'salida',
   TIMEDIFF(asistencia_continuo.salida,asistencia_continuo.entrada) AS totalHR,
   empleado.nombre,
   empleado.apellido,
   empleado.dni,
...
where entrada BETWEEN '$fechainicio' and '$fechafinal' and TIMEDIFF(asistencia_continuo.salida,asistencia_continuo.entrada)<'08:00:00'order by id_empleado asc
");
}else{
      $sql = $conexion->query("SELECT
      asistencia_continuo.id_asistenciac,
      ...
      INNER JOIN empleado ON asistencia_continuo.id_empleado = empleado.id_empleado
      INNER JOIN cargo ON empleado.cargo = cargo.id_cargo
      where asistencia_continuo.id_empleado=$empleado and entrada BETWEEN '$fechainicio' and '$fechafinal' and TIMEDIFF(asistencia_continuo.salida,asistencia_continuo.entrada)<'08:00:00' order by id_asistenciac asc");
}
